...
// Función para crear un ítem de publicación del blog
const createBlogPostItem = (post, index) => {
  ...
  bannerImage.src = post.imageSrc;
  bannerImage.alt = post.imageAlt;
  ...
  blogCategory.innerText = post.category;
  ...
  blogMeta.appendChild(postDate);
  blogContent.appendChild(blogMeta);
  blogContent.appendChild(blogTitle);
  blogContent.appendChild(blogText);

  postLink.appendChild(bannerBox);
  postLink.appendChild(blogContent);
  postItem.appendChild(postLink);

  return postItem;
};

// Cargar los datos del blog y mostrarlos
fetch('./blog-data.json')
  .then(response => response.json())
  .then(data => {
    const blogPostsList = document.querySelector('.blog-posts-list');
    data.forEach((post, index) => {
      const postItem = createBlogPostItem(post, index);
